...
public class BezierDarstellung implements RealFieldUnivariateFunction<Dfp> {
    ...
    public Dfp value(Dfp x) {
        return deCasteljau(mu(x, s, t), b.length - 1, 0);
    }
    ...
    public Dfp derivative(Dfp x, int m) {
        ...
        Dfp mu = mu(x, s, t);
        switch (m) {
            case 0 -> tempValue = value(x);
            case 1 -> tempValue = (t.subtract(s)).reciprocal()
                    .multiply(b.length - 1).multiply(deCasteljau(mu, b.length
                            - 2, 1).subtract(deCasteljau(mu, b.length - 2, 0)));
            case 2 -> tempValue = (t.subtract(s).pow(2)).reciprocal()
